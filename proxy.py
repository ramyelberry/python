...
if len(sys.argv) != 2:
    print
    "Usage: python %s <PROXY_PORT>" % sys.argv[0]
    print
    "Example: python %s 20000" % sys.argv[0]
    raise SystemExit
...
def do_cache_or_not(fileurl):
    try:
        ...
        if datetime.datetime.fromtimestamp(time.mktime(last_third)) + datetime.timedelta(
...
def serve_get(client_socket, client_addr, details):
    try:
        ...
        if last_mtime and "304 Not Modified" in reply:
            print
            "returning cached file %s to %s" % (cache_path, str(client_addr))
            get_access(details["total_url"])
            ...
            if do_cache:
                print
                "caching file while serving %s to %s" % (cache_path, str(client_addr))
                ...
                print
                "without caching serving %s to %s" % (cache_path, str(client_addr))
                # print len(reply), reply
                while len(reply):
                    client_socket.send(reply)
                    reply = server_socket.recv(BUFFER_SIZE)
...
def start_proxy_server():
    ...
    try:
        ...
        print
        "Error in starting proxy server ..."
        print
        e
        proxy_socket.close()
        raise SystemExit
